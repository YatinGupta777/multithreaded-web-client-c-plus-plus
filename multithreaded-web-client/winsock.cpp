...
void WebScrapping::connect_and_parse(char*& buffer, int port, const char* method, char* host, char* path, char* query, char* link, int status_code_validation, int &curr_pos, int max_size)
{
	start_t = clock();
...
void WebScrapping::get_request(int port, char* host, char* path, char* query, char* link) {
	if (!error) connect_and_parse(get_buffer, port, "GET", host, path, query, link, 200, get_buffer_size, 2000000);
}
